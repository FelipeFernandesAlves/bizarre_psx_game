...
func _physics_process(delta: float) -> void:
	...
	if curr_looking_obj != looking_obj:
		if looking_obj && looking_obj.has_signal("unfocused"): 
	...
	if looking_obj && looking_obj.has_signal("interacted") && Input.is_action_just_pressed("interact"):
